...
def parse_time_tracking_file(file_path: Path) -> Dict[str, Dict[str, any]]:
    ...
    for i, day_match in enumerate(day_matches):
        ...
        if i + 1 < len(day_matches):
            ...
            day_end = day_start + next_section.start() if next_section else len(content)
...
def parse_day_tasks(day_content: str) -> Dict[str, any]:
    ...
    for line in lines:
        ...
        if line.startswith('- ') and ':' in line and re.search(r'\d{2}:\d{2}', line):
            if current_task:
        ...
        # Sub-bullet point (notes)
            ...
            current_task['notes'].append(line[4:])  # Remove '  - '
    ...
    for i, task in enumerate(tasks):
        ...
        if i + 1 < len(tasks):
            ...
            # Assume last task ends at 18:00 if not specified
            end_time = 18 * 60  # 18:00 in minutes
...
def print_summary(results: Dict[str, Dict[str, any]]):
    ...
    for date, tasks in results.items():
        ...
        sorted_tasks = sorted(tasks.items(), key=lambda x: x[1]['total_duration'], reverse=True)
...
def main():
    parser = argparse.ArgumentParser(description='Parse time tracking markdown files')
    parser.add_argument('files', nargs='+', help='Markdown files to parse')
    parser.add_argument('--summary', action='store_true', help='Show summary across all files')
        ...
        if args.summary and len(all_results) > 1:
            ...
            for date, tasks in all_results.items():
                for task_name, task_data in tasks.items():
                    ...
                    overall_tasks[task_name]['notes'].extend(task_data['notes'])
                    overall_tasks[task_name]['days'] += 1
            
            # Sort by total duration
            sorted_overall = sorted(overall_tasks.items(), key=lambda x: x[1]['total_duration'], reverse=True)
            
            for task_name, task_data in sorted_overall:
                duration_str = minutes_to_duration(task_data['total_duration'])
                days_str = f"({task_data['days']} day{'s' if task_data['days'] > 1 else ''})"
